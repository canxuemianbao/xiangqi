...
int CBoard::AlphaBetaSearch(int depth, int alpha, int beta)	// Alpha-Beta搜索算法
{
	...
	value = ReadHashTable(depth,alpha,beta);
	if (value != NOVALUE) 
	{
		return value;
	}
	...
	{
		value = Eval();
		SaveHashTable(value,depth,hashEXACT);
		return value;
	}
	...
	for(i = 0 ; i<num; i++)
	{
		...
		if(value >= beta)
		{
			SaveHashTable(beta,depth,hashBETA);
			return beta;
		}			
		if(value > best)
		{
	...
	if(alphaFlag)
		SaveHashTable(alpha,depth,hashALPHA);	
	else
		SaveHashTable(best,depth,hashEXACT);	
