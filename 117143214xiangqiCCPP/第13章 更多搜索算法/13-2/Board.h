...
#ifndef CLASS_CBOARD_H
#define CLASS_CBOARD_H
...
public:
	...
	move bokList[MAX_BOOK_POS][MAX_BOOK_MOVE];        // 开局库附加表
	int bokListNum;		//附加表的数目,即附加表中的已存放的数据数,必须<=MAX_BOOK_POS
	...
	int GenCapMove(move * MoveArray);	//生成所有的走法
	int GenNonCapMove(move * MoveArray);	//生成所有的走法
	...
	int IsSafeMove (move mv); //安全的走法，即走后不被对手将军
	int Protected(int lSide,unsigned char dst);	//检测lSide一方能否保护位置dst,能返回1，否则返回0
	...
	// 搜索函数------------------------------------------
	void ComputerThinkTimer(int TimeLimit);	//电脑思考，按时间
	void ComputerThinkDepth(int depth=5);	//电脑思考，按步数
	int AlphaBetaSearch(int depth, int alpha, int beta);	// Alpha-Beta搜索算法
	int QuiescentSearch (int alpha, int beta);	//静态搜索算法
	bool MakeMove(move m);	// 执行走法
	void UnMakeMove();		// 撤消走法
	void ChangeSide();		// 交换走棋方
	int MDTFSearch(int depth,int f,int alpha,int beta);	//MDF搜索算法

	//哈希表相关函数 在hash.cpp中------------------------------------
	void IniZobrist();	//生成键值和校验值函数
	void ClearHashTable();	//清空哈希表
	int ReadHashTable(int depth, int alpha, int beta, move &mv);	//读哈希表,成功返回局面的估值，失败返回NOVALUE
	void SaveHashTable(int value, int depth, int type, move mv);	//写哈希表，将当前局面估值value写入哈希表
	void NewHashTable();	//生成哈希数组
	void DelHashTable();	//销毁哈希数组

	//开局库处理函数
	void LoadBook(const char *szBookFile); // 把开局库装入到置换表中
	move GetBookMove(void);          // 从置换表中获取开局库着法
