...
int CBoard::MDTFSearch(int depth,int f,int alpha,int beta)
{
	int low = alpha;
	int high = beta;
	while(low < high)
	{
		...
		f = AlphaBetaSearch(depth, beta-1, beta);
		if(f<beta)
			high = f;
		else
			low = f;
...
void CBoard::ComputerThinkTimer(int TimeLimit)
{
	...
	{
		f = MDTFSearch(MaxDepth,f,-MaxValue, MaxValue);
		if(sg_TimeOut)
			break;
		else
			BackupMove = BestMove; 
...
void CBoard::ComputerThinkDepth(int depth)
{
	...
	MaxDepth = depth;
	sg_DepthTimer = 0;
	MDTFSearch(MaxDepth,f,-MaxValue, MaxValue);
	BackupMove = BestMove; 
